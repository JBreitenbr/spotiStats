...
canvas.append('g').style("font",w<580?"8px arial":"13px arial").call(xAxis).attr('transform','translate(0,'+(h-pad-yScale.bandwidth())+')');
if(w<580){
canvas.append('g').style("font","8px arial").call(yAxis).attr('transform','translate('+2.4*pad+','+ (-1)*yScale.bandwidth()+')');
}
else {
canvas.append('g').style("font",w<700?"11px arial":w<900?"14px arial":"16px arial").call(yAxis).attr('transform','translate('+1.9*pad+','+ (-1)*yScale.bandwidth()+')');
}
